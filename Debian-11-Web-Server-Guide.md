...
- [Debian 11 Web Server Guide](#debian-11-web-server-guide)
  ...
  * [MariaDB](#mariadb)
    + [About](#about-4)
    + [Installation & Setup](#installation---setup-3)
      - [Update Debian System](#update-debian-system-1)
      - [Install Required Dependencies](#install-required-dependencies)
      - [Install curl (If not already installed)](#install-curl--if-not-already-installed-)
      - [Import MariaDB 10.8 GPG Keys & Repository](#import-mariadb-108-gpg-keys---repository)
      - [Install or Upgrade MariaDB](#install-or-upgrade-mariadb)
      - [Verify MariaDB version installed and check status](#verify-mariadb-version-installed-and-check-status)
      ...
      - [Remove bash script repository](#remove-bash-script-repository)
  * [PHPMyAdmin](#phpmyadmin)
    + [Important Notice](#important-notice)
    + [Requirements](#requirements)
      - [mbstring Apache Module](#mbstring-apache-module)
    + [Installation & Setup](#installation---setup-4)
      - [Install PHP extensions required for PHPMyAdmin](#install-php-extensions-required-for-phpmyadmin)
      ...
      - [Move the default phpMyAdmin installation folder and rename folder](#move-the-default-phpmyadmin-installation-folder-and-rename-folder)
      - [Depending on the version installed of your phpmyadmin, you may need to type in your folder name manually in the command below.](#depending-on-the-version-installed-of-your-phpmyadmin--you-may-need-to-type-in-your-folder-name-manually-in-the-command-below)
      - [Create temporary files folder for phpMyAdmin](#create-temporary-files-folder-for-phpmyadmin)
      - [Give ownership of phpMyAdmin folder to www-data user group](#give-ownership-of-phpmyadmin-folder-to-www-data-user-group)
      - [Make a config file using a copy of the config. Sample](#make-a-config-file-using-a-copy-of-the-config-sample)
      - [Secure phpMyAdmin with secret passphrase](#secure-phpmyadmin-with-secret-passphrase)
      - [Open phpMyAdmin’s configuration file](#open-phpmyadmin-s-configuration-file)
      - [Enter or modify the following](#enter-or-modify-the-following)
      - [To Save the file in Nano, then close](#to-save-the-file-in-nano--then-close)
  * [Accessing PHPMyAdmin](#accessing-phpmyadmin)
    + [Notes](#notes-2)
      - [Login Debugging](#login-debugging)
  * [User Management](#user-management)
    + [Basic Setup on Linux Server](#basic-setup-on-linux-server)
      - [Give a non-root account sudo privileges](#give-a-non-root-account-sudo-privileges)
      - [Change ownership of users own folder from root to self](#change-ownership-of-users-own-folder-from-root-to-self)
      - [Give file write permissions to users SSH folder](#give-file-write-permissions-to-users-ssh-folder)
      - [To understand what 755 is doing with chmod.](#to-understand-what-755-is-doing-with-chmod)
    + [Secure Shell (SSH) & Secure File Transfer Protocol (SFTP) setup](#secure-shell--ssh----secure-file-transfer-protocol--sftp--setup)
      - [Generating an SSH Key with Windows Powershell](#generating-an-ssh-key-with-windows-powershell)
        * [RSA Options](#rsa-options)
      - [PuTTY](#putty)
      - [Throwing SSH Public Key up to Linux server](#throwing-ssh-public-key-up-to-linux-server)
      - [Template SSH Login over Windows Powershell](#template-ssh-login-over-windows-powershell)
    + [Accessing the Server through local terminal or PowerShell](#accessing-the-server-through-local-terminal-or-powershell)
      - [Throwing SSH Key from Local System to Server](#throwing-ssh-key-from-local-system-to-server)
  ...
  * [Applying Folder Permissions](#applying-folder-permissions)
    + [Single SFTP User](#single-sftp-user)
      - [Assign Permissions to directories](#assign-permissions-to-directories)
      - [Give Root ownership to directories](#give-root-ownership-to-directories)
    + [Group sharing access.](#group-sharing-access)
      - [Create the user group](#create-the-user-group)
      - [Assign user(‘s) to group](#assign-user--s--to-group)
      - [Make a company or user folder for the SFTP user](#make-a-company-or-user-folder-for-the-sftp-user)
      - [Assign Permissions to directories](#assign-permissions-to-directories-1)
      - [Make a projects folder](#make-a-projects-folder)
      - [Assign Permissions to projects](#assign-permissions-to-projects)
      - [Change group ownership of directory](#change-group-ownership-of-directory)
  * [SSHD_Config SSH & SFTP Settings](#sshd-config-ssh---sftp-settings)
    + [Open file in nano](#open-file-in-nano)
    + [Settings](#settings)
  * [Debug Notes](#debug-notes-1)
    + [WinSCP](#winscp)
